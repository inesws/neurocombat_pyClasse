...
# neurocombat function used in this code has been:
# Originally written by Nick Cullen
# Extended and currently maintained by JP Fortin in his repository https://github.com/Jfortin1/neuroCombat

# Here it has been modified and extended by me:
# - a neurocombat_transform function that applies combat parameters in every case, also if reference batch option has been chosen (M-ComBat version)
# - - when working in M-ComBat mode:
# - - - in _transform, it checks whether data is the training set (where estimations have been performed) or a new set
# - - - in neurocombat_estimate the mod_mean is calculated based on the reference with the rest of the given data in order to estimate properly the site effects
# - - - in neurocombat_transform the mod_mean applied is calculated based on the reference batch and not using other data covariates
